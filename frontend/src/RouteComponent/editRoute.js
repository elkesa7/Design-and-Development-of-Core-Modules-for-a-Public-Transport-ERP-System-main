...
export default function EditRoute() {
  ...
  useEffect(() => {
    Axios.get(`http://localhost:1234/route/${_id}`).then((res) => {
      setForm({
        id_interne: res.data.data.id_interne,
        nb_stations: res.data.data.nb_stations,
        nature_route: res.data.data.nb_stations,
        centre: res.data.data.centre,
        point_depart: res.data.data.point_depart,
        destination: res.data.data.destination,
        distance: res.data.data.distance,
        heures_depart: res.data.data.heures_depart,
        heures_arrivee: res.data.data.heures_arrivee,
      });
    });
  }, []);
  ...
  const handle = () => {
    try {
      Axios.patch(`http://localhost:1234/route/${_id}`, form)
        .then(() => {
          navigate("/route");
        })
        .catch((e) => {
  ...
  return (
    <div style={wrapperStyle}>
      ...
      <div style={rowStyle}>
        ...
        <div style={fieldStyle}>
          <label style={labelStyle}>
            Nombre stations <span style={requiredStyle}>*</span>
          </label>
          <input
            style={inputStyle}
            type="number"
