...
export default function AddMouvement() {
      ...
      Axios.get("http://localhost:1234/personnel/")
        .then((res) => {
          const chauffeur_affecte = res.data.data
            ...
            .map((personnel) => ({
              _id: personnel._id,
              nom_prenom: personnel.prenom + " " + personnel.nom,
            }));
  ...
  const handle = () => {
    try {
      Axios.post("http://localhost:1234/mouvement/", form)
        .then(() => {
          navigate("/mouvement");
        })
        .catch((e) => {
